...
class MenuService:
    ...
    async def get_menu(self, menu_id: UUID) -> Menu | None:
        ...
        if cached_menu:
            ...
            db_menu = await self.menu_repository.get_menu_by_id(
                menu_id=menu_id,
            )
        if not db_menu:
            return None
    ...
    async def update_menu(self, menu_id: UUID, menu: MenuCreateUpdate) -> Menu | None:
    ...
    async def delete_menu(self, menu_id: UUID) -> dict | None:
        db_menu = await self.menu_repository.get_menu_by_id(menu_id=menu_id)
        if db_menu:
